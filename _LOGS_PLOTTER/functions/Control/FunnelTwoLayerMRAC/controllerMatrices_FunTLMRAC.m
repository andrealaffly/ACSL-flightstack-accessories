function controller_matrices = controllerMatrices_FunTLMRAC(vp, gains)
  ...
  controller_matrices.P_translational = ...
    lyap(gains.ADAPTIVE.A_transient_translational', gains.ADAPTIVE.Q_translational);
  
  % Solve the continuous Lyapunov equation to compute P_rotational
  controller_matrices.P_rotational = ...
    lyap(gains.ADAPTIVE.A_transient_rotational', gains.ADAPTIVE.Q_rotational);

  % Initialize Projection Operator parameters
  controller_matrices = initializeProjectionEllipsoidParams_TLMRAC(gains, controller_matrices);
