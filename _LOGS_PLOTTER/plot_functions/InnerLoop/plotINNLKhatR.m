function plotINNLKhatR(log, der, pp, gains)
% Plot INNER LOOP Adaptive Gain K_hat_r

set(figure,'Color','white','WindowState','maximized')
plot(log.time,log.inner_loop.K_hat_r.ind00,'-','LineWidth',1.2)
hold on
plot(log.time,log.inner_loop.K_hat_r.ind10,'-','LineWidth',1.2)
plot(log.time,log.inner_loop.K_hat_r.ind20,'-','LineWidth',1.2)

plot(log.time,log.inner_loop.K_hat_r.ind01,'--','LineWidth',1.2)
plot(log.time,log.inner_loop.K_hat_r.ind11,'--','LineWidth',1.2)
plot(log.time,log.inner_loop.K_hat_r.ind21,'--','LineWidth',1.2)

plot(log.time,log.inner_loop.K_hat_r.ind02,'-.','LineWidth',1.2)
plot(log.time,log.inner_loop.K_hat_r.ind12,'-.','LineWidth',1.2)
plot(log.time,log.inner_loop.K_hat_r.ind22,'-.','LineWidth',1.2)

% Determine outer_bound and inner_bound based on available gain structure
if isfield(gains, 'ADAPTIVE')
    S_diag = gains.ADAPTIVE.S_diagonal_r_rotational;
    alpha = gains.ADAPTIVE.alpha_r_rotational;
elseif isfield(gains, 'S_diagonal_r_rot')
    S_diag = gains.S_diagonal_r_rot;
    alpha = gains.alpha_r_rot;
else
    error('Unknown gain structure: Cannot find S_diagonal_r_rotational or S_diagonal_r_rot.');
end
...
proj_op_activated = log.inner_loop.proj_op_activated_K_hat_r;  % Boolean vector
time = log.time;
...
ylabel('Inner loop Adaptive gain $\hat{K}_r$ [-]','interpreter','latex','fontsize',pp.font_size)
